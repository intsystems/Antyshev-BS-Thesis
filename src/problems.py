...
def generate_sym(dim):
    '''
    Generates symmetric matrx of given dimensionality
    '''
    a = mt.rand(1, dim)
...
def gen_cond(n, cond):
    ...
    exp_vec = np.arange(-log_cond/4., log_cond * (n) / (4*(n - 1)), log_cond / (2.*(n-1)))
...
class BaseSaddle(object):
    '''
    Implements SaddleProblem class
    '''
